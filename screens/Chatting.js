...
const Chatting = ({ route, navigation }) => {
  ...
  const fetchComments = async () => {
    ...
    const response = await fetch('http://localhost:3000/api/auth/getTrainerRequests', {
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    const fetchTrainerId = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/auth/getTrainerRequests/${user_id}`);
        const data = await response.json();
  ...
  const postComment = async (comment) => {
    ...
    try {
      ...
      console.log("postComment body", user_id, comment)
      const response = await fetch('http://localhost:3000/api/auth/sendCoaching', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token,
